...
const Game = ({ handleReturn }) => {
  ...
  const stand = () => {
    ...
    setMessage(result[0]);
    setBudget(result[1] && parseInt(budget) + parseInt(bet) * 2);
  ...
  return (
    <Wrapper>
      ...
      <Container fluid className={`${classes.game} text-center`} style={style}>
        ...
        <Row>
          <GameControl gameOver={gameOver} startGame={startGame} hit={hit} stand={stand} budget={budget} setBudget={setBudget} />
        </Row>
        ...
        {gameOver && <footer className={`${classes["back-to-menu"]}`}>
